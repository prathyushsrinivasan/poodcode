# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Stage 4 — Numbers, bits and grids.
#
# exec()'d by tools/dsa_curriculum.py inside its namespace.
#
# Three units that look like trivia and are not. Number theory is where the
# *size of a number* and the *size of the input* part ways, and — since the
# syllabus audit — where "return the answer modulo 10^9 + 7" is taught, because
# every counting DP in stage 7 assumes it. Bit manipulation is the array
# patterns applied to 32 flags in one int, and the representation bitmask DP is
# built on. Simulation closes the stage with no new technique at all: index
# arithmetic under pressure, which is what makes the grid traversals of stage 6
# tractable.
#
# This used to be the back half of a stage called "Order, Numbers & Bits". It
# is its own stage now that recursion, greedy and intervals joined sorting and
# binary search in stage 3, which would otherwise have held eight units.
# ---------------------------------------------------------------------------

_S4 = _stage(
    "numbers-and-grids", "Numbers, Bits & Grids", "🧮",
    "Look inside the values, and index the grid exactly.",
    """
Up to here a number has been something to compare. This stage looks inside it.

**Number theory** is where an O(√n) or O(log n) idea replaces a loop over every
value — the first time the *size of a number* and the *size of the input* clearly
part ways — and where you learn to keep a huge count exact **modulo a prime**,
which every counting problem in the DP stage expects. **Bit manipulation** is the
same set of array patterns applied to 32 flags packed into one `int`, which is how
...
_unit(
    "math-number-theory", "Math & Number Theory", "🧮", _S4,
    "Divisors, primes, gcd — and huge counts kept exact modulo a prime.",
    weight=2,
...
*"The answer can be very large; return it modulo 10⁹ + 7."* That sentence is in
every counting problem, and it means: never let a value leave `[0, p)`.

Addition and multiplication distribute over `%`, so reduce after **every**
operation:

```java
static final long MOD = 1_000_000_007L;
sum  = (sum + x) % MOD;
prod = prod * x % MOD;              // both operands below p < 2³⁰, so the product fits in long
```

Subtraction needs care: `(a - b) % MOD` can be negative in Java, so write
`((a - b) % MOD + MOD) % MOD`.

**Powers** use square-and-multiply, reducing the base *before* the first square
— `(10¹⁸)²` does not fit in anything:
...
**Division** does not distribute over `%`. When p is prime, Fermat's little
theorem gives every nonzero x an inverse, `x^(p−2) mod p`, so `a / b` becomes
`a · power(b, p − 2)`. For many binomial coefficients, precompute factorials once
and the inverse factorials walking *down* from a single inverse:
...
long nCr(int n, int r) {
    return r < 0 || r > n ? 0 : fact[n] * invFact[r] % MOD * invFact[n - r] % MOD;
}
```
""",
    signals=[
        ...
        _sig("“number of ways to choose …”, many queries", "Factorials and inverse factorials mod p",
             "O(N) once, then O(1) per C(n, r)."),
        _sig("A division inside a modular answer", "Multiply by the inverse, x^(p−2)",
             "Only because p is prime — Fermat's little theorem."),
    ],
    skeletons=[
...
""",
        ...
        _sk("Power and inverse modulo a prime",
            "a^b mod p, and division mod p.",
            """
static final long MOD = 1_000_000_007L;

static long power(long b, long e) {
    long r = 1;
    b %= MOD;                                  // BEFORE the first square
    for (; e > 0; e >>= 1, b = b * b % MOD)
        if ((e & 1) == 1) r = r * b % MOD;
    return r;
}

static long inverse(long x) { return power(x, MOD - 2); }   // p prime, x % p != 0
""",
            "Reduce after every product; the product of two values below p always fits in a long."),
        _sk("nCr modulo a prime",
            "Many binomial coefficients up to N.",
            """
...
long nCr(int n, int r) {
    if (r < 0 || r > n) return 0;
    return fact[n] * inv[r] % MOD * inv[n - r] % MOD;   // reduce between the two products
}
""",
            "One O(log p) inverse; every other inverse factorial is a multiplication."),
    ],
    costs=[
        ...
        _cost("Modular inverse by Fermat", "O(log p)", "O(1)", "≈ 30 squarings for p = 10⁹ + 7."),
        _cost("Factorial tables to N", "O(N)", "O(N)", "Then O(1) per binomial coefficient."),
    ],
    pitfalls=[
        ...
        _pit("A modular answer is wrong only on large inputs",
             "A product like `fact[n] * inv[r] * inv[n - r]` overflowed before the single `% MOD` at the end.",
             "Reduce after every multiplication: `fact[n] * inv[r] % MOD * inv[n - r] % MOD`."),
        _pit("`power(a, b)` is wrong for a near 10¹⁸",
             "The base was squared before being reduced, and `a * a` overflowed.",
             "`b %= MOD` first."),
        _pit("A division gives a wrong modular answer",
             "`(a / b) % MOD` was computed on already-reduced values, where integer division means nothing.",
             "Multiply by the modular inverse instead: `a * power(b, MOD - 2) % MOD`."),
    ],
    ...
    checks=[
        ...
        _chk("Why can two values below 10⁹ + 7 be multiplied in a `long` without overflow?",
             "Both are below 2³⁰, so the product is below 2⁶⁰ < 2⁶³. That is also why you must "
             "reduce after *each* multiplication — a third factor would not fit."),
        _chk("How do you divide modulo a prime p?",
             "Multiply by the inverse. By Fermat's little theorem x^(p−1) ≡ 1, so x^(p−2) is x's "
             "inverse for any x not divisible by p."),
        _chk("Given fact[] and invFact[N], how do you get every other invFact cheaply?",
             "Walk down: 1/(i−1)! = i · 1/i!, so invFact[i − 1] = invFact[i] · i mod p. One "
...
Modular arithmetic is the opposite: rarely the point, constantly required. A
counting solution that overflows on the large test is a failed solution, so
reduce after every operation without being asked.
""",
    rungs=[
...
_unit(
    "bit-manipulation", "Bit Manipulation", "🔟", _S4,
...
# --- Unit 18 — Simulation and matrices ---------------------------------------

_unit(
    "simulation-and-matrix", "Simulation & Matrices", "🎛️", _S4,
...
vocabulary that every grid traversal in stage 6 assumes you already have.
""",
    model="""
### Grid vocabulary
...
""",
    rungs=[
        ...
        _rung("Core", "Neighbour counting on a grid.",
              ...
              {"minesweeper-counts": "Eight neighbours, bounds-checked. This loop is the ancestor of every flood fill in stage 6."}),
