...
A unit page is split into four tabs, in the order a unit is worked — **📖 Learn**
(why, the model, what is underneath, traces), **🧰 Toolkit** (signals, the
playbook, costs, pitfalls), **🧗 Practice** (the ladder) and **🔁 Review**
(self-checks, Big-O drills, interview notes) — and each tab ends by handing you
to the next.

Two beats are on **every** unit:

- **⏳ Price the snippet** — five graded Big-O questions per unit (`tools/dsa_bigo.py`),
  each a trap specific to that technique: the `while` inside a sliding window's
  `for` that is still linear, `list.contains` inside a loop, a recursion that
  halves `e` and still costs O(e) because it calls itself twice. They are
  scheduled like the self-checks, and every answer comes with the reason and the
  fix.
- **🎞️ A worked trace** — at least one per unit. The ones in
  `tools/dsa_traces.py` are *generated by running the algorithm* on the example,
